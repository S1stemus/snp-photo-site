...
class UserShowView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        outcome = ServiceOutcome(
            UserShowService, {"user_id": kwargs["id"]}, {"current_user": request.user}
        )
    ...
    def put(self, request, *args, **kwargs):
        outcome = ServiceOutcome(
            UserUpdateService,
            {"user_id": kwargs["id"], "current_user": request.user}
            | request.data.dict(),
            request.FILES,
        )
        return Response(UserSerializer(outcome.result).data)
...
class RegisterUserView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        outcome = ServiceOutcome(
            CreateUserService(), request.data.dict(), request.FILES
        )
