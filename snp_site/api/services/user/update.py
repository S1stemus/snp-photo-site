...
class UserUpdateService(ServiceWithResult):
    ...
    def _validate_user(self):
        if self.cleaned_data['current_user'] != User.objects.get(id=self.cleaned_data['user_id']):
            self.add_error('current_user', 'Вы не можете редактировать профиль другого пользователя')
