...
class CreateCommentService(ServiceWithResult):
    ...
    def _create_comment(self):
        comment = Comment.objects.create(user=self.cleaned_data['current_user'], comment=self.cleaned_data['comment'], content_type=ContentType.objects.get_for_model(self.class_dict[self.cleaned_data['content_type']]), object_id=self.cleaned_data['object_id'])
        return comment
