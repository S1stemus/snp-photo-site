    ...
    def _validate_comment_exist(self):
        if not self._comment:
            self.add_error(
                "id",
                NotFound(
                    message=f'Комментарий с id {self.cleaned_data["id"]} не найден'
                ),
            )
    ...
    def _validated_user(self):
        if self.cleaned_data["current_user"].id != self._comment.user.id:
            self.add_error(
                "current_user",
                NotFound(
                    message=f'пользователь {self.cleaned_data["current_user"]} не может удалить этот комментарий'
                ),
            )
    ...
    def _validate_comments_exist(self):
        if self._comment.model_relation.all().count() != 0:
            self.add_error(
                "id",
                NotFound(
                    message=f'Комментарий с id {self.cleaned_data["id"]} имеет комментарии'
                ),
            )
